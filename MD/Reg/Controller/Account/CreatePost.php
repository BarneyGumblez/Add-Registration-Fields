...
namespace MD\Reg\Controller\Account;
...
class CreatePost extends AbstractAccount implements CsrfAwareActionInterface, HttpPostActionInterface
{
    ...
    public function execute()
    {
        ...
        if($this->getRequest()->getParam('md-reg-checkbox')){
            $data = $this->transformDataToArray($this->getRequest()->getParam('md-reg-checkbox'), $data);
        }
        if($this->getRequest()->getParam('md-reg-input')){
            $data['input'] = $this->getRequest()->getParam('md-reg-input');
        }
        if($this->getRequest()->getParam('md-reg-select')){
            $data['select'] = $this->getRequest()->getParam('md-reg-select');
        }

        $this->_request->setPostValue('other_data', serialize($data));
    ...
    public function transformDataToArray($element, $data){
        if($element) {
            $checkbox = $element;
            foreach ($checkbox as $elem => $val) {
                $answer = substr($elem, strpos($elem, '=>') +2, strlen($elem));
                $ask = stristr($elem, '=>', true);
                if(in_array($ask, $data['checkbox']) == false){
                    $data['checkbox'] += array(
                        $ask => array()
                    );
                    if(in_array($answer, $data['checkbox'][$ask]) == false){
                        array_push($data['checkbox'][$ask], $answer);
                    }
                }
            }
        }
